...
class OceanConnection(BaseSyncConnection):
    ...
    def _download_asset(self, envelope: Envelope):
        ...
        if data_token.balanceOf(self.wallet.address) <= envelope.message.datatoken_amt * 1.05:
            self.logger.info(
        ...
        results = self._download(asset, service, order_tx_id)

        if len(results) == 0:
            self.logger.error(f"Failed to download. retrying....")
            retries = 0
            while retries < 3 or len(results) == 0:
                results = self._download(asset, service, order_tx_id)
                retries +=1
                self.logger.error(f"retry {retries}")
            if len(results) == 0:
                data = b""
        ...
        self.logger.info(f"completed download! Sent {len(results)} results to handler!")
        
    def _download(self, asset, service, order_tx_id):
        file_path = self.ocean.assets.download(
            asset.did,
            service.index,
            self.wallet,
    ...
    def _deploy_datatoken(self, envelope: Envelope):
        ...
        self.logger.info(f"datatoken.address = '{datatoken.address}'")
        return datatoken
